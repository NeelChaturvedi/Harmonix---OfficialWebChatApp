    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger className="focus:outline" asChild>
                <button className="w-full text-md font-semibold px-3 flex items-center h-12 border-neutral-200 dark:border-sky-600 border-b-2 hover:bg-zinc-700/10 dark:hover:bg-blue-500 transition">
                    {server.name}
            ...
            <DropdownMenuContent className="w-56 text-xs font-medium text-black dark:text-neutral-400 space-y-[2px]">
                {isModerator && (
                    <DropdownMenuItem onClick={() => onOpen("invite", { server })} className="text-cyan-600 dark:text-cyan-400 px-3 py-2 text-sm cursor-pointer">
                        Invite People
